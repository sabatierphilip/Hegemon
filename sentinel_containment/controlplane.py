...
class HegemonControlPlane:
    ...
    def _build_poc_bundle(self, endpoint: Endpoint, cve: str, evidence: list[dict[str, Any]], reasoning: str) -> dict[str, Any]:
        ast_issue = next((e for e in evidence if e.get("type") == "ast_issue"), {})
        chain = ast_issue.get("reconstructed_kill_chain") or ["recon", "execution", "impact"]
        call_path = ast_issue.get("call_path") or [endpoint.host_name, cve]
        dataflow_path = ast_issue.get("dataflow_path") or ["input", "vulnerable_sink"]
        issue_id = str(ast_issue.get("issue_id", "generic"))

        exploit_primitives = {
            "tainted-cmd-exec": ["inject shell metacharacters", "gain command execution", "persist via cron/systemd"],
            "tainted-sql-query": ["inject SQL predicate", "extract credentials/session table", "pivot to admin"],
            "hardcoded-secret": ["extract constant from artifact", "reuse token on API", "enumerate privileged actions"],
            "weak-rng-auth": ["predict token stream", "forge session", "elevate privileges"],
            "dynamic-exec": ["deliver crafted payload", "trigger dynamic execution", "execute arbitrary code"],
        }
        primitives = exploit_primitives.get(issue_id, ["craft exploit input", "trigger vulnerable path", "observe impact"])

        validation_checks = [
            "reproduce in isolated canary sandbox",
            "assert exploit fails after patch",
            "run regression + integration + sbom reproducibility checks",
            "verify telemetry/markov chain risk decreases post-patch",
        ]

        remediation_plan = [
            "apply minimal code fix on vulnerable sink/source path",
            "deploy canary with rollback-on-anomaly",
            "run exploit replay and negative tests",
            "promote patch ring-by-ring with runtime attestation",
        ]

        graph_nodes = [
            {"id": "source", "label": str(dataflow_path[0])},
            {"id": "sink", "label": str(dataflow_path[-1])},
            {"id": "asset", "label": endpoint.host_name},
            {"id": "vuln", "label": cve},
        ]
        graph_edges = [
            {"source": "source", "target": "sink", "type": "dataflow"},
            {"source": "sink", "target": "asset", "type": "executes_on"},
            {"source": "asset", "target": "vuln", "type": "exposes"},
        ]

        return {
            "schema": "hegemon.poc_bundle.v1",
            "issue_id": issue_id,
            "reconstructed_kill_chain": chain,
            "call_path": call_path,
            "dataflow_path": dataflow_path,
            "exploit_primitives": primitives,
            "poc_graph": {"nodes": graph_nodes, "edges": graph_edges},
            "validation_checks": validation_checks,
            "remediation_plan": remediation_plan,
            "operator_explanation": (
                f"PoC plan generated for {cve}: chain={' -> '.join(str(v) for v in chain)}; "
                f"call_path={' -> '.join(str(v) for v in call_path)}; reasoning={reasoning}"
            ),
        }
    ...
    def create_finding(self, payload: dict[str, Any], actor: str) -> VulnerabilityFinding:
        ...
        poc_bundle = payload.get("poc_bundle") or self._build_poc_bundle(endpoint, str(payload.get("cve", "unknown")), evidence, reasoning)
        if not any(e.get("type") == "poc_bundle" for e in evidence):
            evidence.append({"type": "poc_bundle", **poc_bundle})
    ...
    def generate_patch_proposal(self, finding_id: str, actor: str) -> PatchProposal:
        ...
        poc_bundle = next((e for e in finding.evidence if e.get("type") == "poc_bundle"), {})
        if poc_bundle:
            change_plan = list(change_plan) + ["validate PoC replay map and remediation plan before production promotion"]
        proposal = PatchProposal(
            ...
            reasoning=(
                ...
                f"PoC map: {str(poc_bundle.get('operator_explanation', 'none'))}"
            ),
        )
